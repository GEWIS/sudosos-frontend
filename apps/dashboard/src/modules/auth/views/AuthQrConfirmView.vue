...
const confirmLogin = async () => {
  ...
  try {
    ...
    console.error('Confirming login with sessionId:', sessionId.value);
    const response = await fetch(`/api/v1/authentication/qr/${sessionId.value}/confirm`, {
      method: 'POST',
      headers: {
    ...
    authStore.handleResponse(data, apiService);
...
onMounted(() => {
  ...
  if (sessionIdParam) {
    ...
    // Automatically try to confirm login if sessionId is present
    void confirmLogin();
  } else {
    error.value = t('modules.auth.qr.noSessionId');
  }
