<template>
  ...
  <div v-else-if="props.state == 'Permission'" class="flex flex-column justify-content-between gap-3">
    ...
    >
        ...
        <DataTable
class="w-full" table-style="min-width: 27.5rem" :value="
          ...
          <Column field="delete">
            <template #body="slotProps">
              <Button
                  ...
                  icon="pi pi-trash"
                  type="button"
                  @click="handleDeletePush(props.form.context.values.currentPermission.entity,
              props.form.context.values.id, slotProps.data.action, slotProps.data.relation, slotProps.data.attribute)"
              />
            </template>
          </Column>
  ...
  <Dialog
v-model:visible="permissionVision" :header="t('modules.financial.rbac.permissions.deletePermission')" modal
          :style="{width:'25rem'}">
  ...
  <Dialog
v-model:visible="actionVision" :header="t('modules.financial.rbac.permissions.addAction')" modal
          :style="{width:'25rem'}">

    <div class="flex flex-column justify-content-center align-items-center gap-3">
      <Dropdown
v-model="selectedAction" checkmark editable
                :highlight-on-select="false" :options="testsActions" placeholder="Select an Action"/>

      <Dropdown
v-model="selectedRelation" checkmark editable
                :highlight-on-select="false" :options="testsRelations" placeholder="Select an Relation"/>

      <Dropdown
v-model="selectedAttribute" checkmark editable
                :highlight-on-select="false" :options="testsAttributes" placeholder="Select an Attribute"/>
    </div>
...
watch(() => props.form.context.values.currentPermission ,() => {
  const allRelations : any[] = [];
  props.form.context.values.currentPermission.actions.forEach(action => {
    action.relations.forEach(relation => {
      allRelations.push({ "action": action.action, "relation": relation.relation,
