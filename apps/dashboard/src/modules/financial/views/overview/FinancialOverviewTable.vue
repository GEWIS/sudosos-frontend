<template>
  <DataTable data-key="transferType" :row-class="onRowClass" striped-rows :value="tableData">
    <Column style="width: 3rem">
      <template #body="{ data }">
        <button
          v-if="!data.isSkeleton && !data.isChild && data.children?.length > 0"
          class="expander-btn"
    ...
    <Column class="text-left" header="Type" header-class="font-sans">
      <template #body="{ data }">
        <Skeleton v-if="data.isSkeleton" height="1rem" />
        <span v-else-if="data.isChild" class="pl-6 text-color-secondary">{{ data.name }}</span>
        <span v-else>{{ data.transferType }}</span>
      </template>
    </Column>

    <Column class="font-mono text-right" header="Credit" header-class="font-sans text-right">
      <template #body="{ data }">
        <Skeleton v-if="data.isSkeleton" height="1rem" />
    ...
    <Column class="font-mono text-right" header="Debit" header-class="font-sans text-right">
      <template #body="{ data }">
        <Skeleton v-if="data.isSkeleton" height="1rem" />
    ...
    <Column class="font-mono text-right" header="Saldo" header-class="font-sans text-right">
      <template #body="{ data }">
        <Skeleton v-if="data.isSkeleton" height="1rem" />
    ...
    <Column class="w-2" :header="t('common.actions')" header-class="font-sans">
      <template #body="{ data }">
        <Button
          v-if="!data.isSkeleton && !data.isChild"
    ...
    <ColumnGroup type="footer">
      <Row>
        <Column :colspan="4" footer="Total Mutations (Δ)" />
        <Column class="font-mono text-right" :footer="formatCurrency(totalMutationSaldo)" />
        <Column />
      </Row>

      <Row>
        <Column :colspan="4" :footer="`Balance at ${start}`" />
      ...
      <Row>
        <Column :colspan="4" footer="Expected End Balance (Start + Δ)" />
        <Column class="font-mono text-right">
          <template #footer>
            <Skeleton v-if="loadingBalances" height="1rem" />
      ...
      <Row>
        <Column :colspan="4" :footer="`Actual Balance at ${end}`" />
        <Column
...
const formatCurrency = (value: number) => {
  return new Intl.NumberFormat('en-DE', { minimumFractionDigits: 2 }).format(value);
};
...
const onRowClass = (data: FinancialOverviewTableRow) => {
  if (data.isSkeleton) return 'skeleton-row';
    ...
    if (row.children && row.children.length > 0) {
      ...
      if (isExpanded || isCollapsing) {
        for (const child of row.children) {
          result.push({
            transferType: `${row.transferType}-child-${child.name}`,
            isChild: true,
  ...
  const mainDataFetch = Promise.all([
    ...
    .then(([summaryRes, payoutRes]) => {
      ...
      const categoryRows: FinancialOverviewTableRow[] = [
        { transferType: 'Deposits', credit: amt(s.deposits), debit: 0, children: [] },
        { transferType: 'Invoices', credit: amt(s.invoices), debit: 0, children: [] },
        { transferType: 'Credit Invoices', credit: 0, debit: amt(s.creditInvoices), children: [] },
        { transferType: 'Waived Fines', credit: amt(s.waivedFines), debit: 0, children: [] },
        { transferType: 'Manual Creations', credit: amt(s.manualCreations), debit: 0, children: [] },
        { transferType: 'Fines', credit: 0, debit: amt(s.fines), children: [] },
        { transferType: 'Write-offs', credit: amt(s.writeOffs), debit: 0, children: [] },
        { transferType: 'Admin Costs', credit: 0, debit: amt(s.inactiveAdministrativeCosts), children: [] },
        { transferType: 'Payout Requests', credit: 0, debit: amt(s.payoutRequests), children: [] },
        {
          transferType: 'Manual Deletions',
          credit: 0,
    ...
    .catch((error) => {
      console.error('Error fetching financial mutations:', error);
    })
    .finally(() => {
