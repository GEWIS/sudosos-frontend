...
export const usePointOfSaleStore = defineStore('pointOfSale', {
  ...
  actions: {
    ...
    async getUserPointsOfSale(userId: number, take: number, skip: number) {
      return (await ApiService.user.getUsersPointsOfSale({ id: userId, take, skip })).data;
    },
    ...
    async updatePointOfSale(
      ...
      return await ApiService.pos.updatePointOfSale({ id, updatePointOfSaleRequest: updatePointOfSale }).then((res) => {
