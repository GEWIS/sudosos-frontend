  ...
  <CardComponent class="w-full" :header="t('modules.financial.administrative.eligibleUsers.title')">
    <div class="flex flex-col gap-4">
      ...
      >
        ...
        <Column field="userId" :header="t('common.userId')">
          <template #body="slotProps">
            <Skeleton v-if="isLoading || !slotProps.data" class="w-6 my-1 h-1rem surface-300" />
            <span v-else>{{ slotProps.data.userId }}</span>
          </template>
        </Column>
      </DataTable>

      <div v-if="selectedUserIds.length > 0" class="flex flex-row justify-content-end gap-2">
        <Button
          ...
          @click="handleNotify"
        >
          <ProgressSpinner v-if="isNotifyLoading" class="mr-2 h-1rem w-1rem" />
        </Button>
        <Button
          v-if="mode === 'handout'"
          :disabled="isHandoutLoading"
          :label="t('modules.financial.administrative.eligibleUsers.handout')"
          @click="handleHandout"
        >
          <ProgressSpinner v-if="isHandoutLoading" class="mr-2 h-1rem w-1rem" />
        </Button>
      </div>
    </div>
  </CardComponent>
    ...
    accept: async () => {
      try {
        await store.notifyUsers({
          userIds: selectedUserIds.value.map((u) => u.userId),
        });
        toast.add({
...
async function handleHandout() {
  ...
  confirm.require({
    ...
    acceptLabel: t('modules.financial.administrative.eligibleUsers.handout'),
    rejectLabel: t('common.cancel'),
    accept: async () => {
      try {
